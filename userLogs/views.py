...
def signup(request):
    if request.method == 'POST':
        ...
        if len(username) > 15:
            messages.error(request, "Username must be under 15 characters long")
            return redirect('signup')
...
def signin(request):
    if request.method == 'POST':
        ...
        if myuser is None:
            ...
            return render(request, 'dashboard/home.html', {'fname': myuser.username})
...
def forgot_password(request):
    ...
    return render(request, "template/forgot_pass.html")


def password_reset_done(request):
    return render(request, "template/password_reset_done.html")
    ...
    if user is not None and default_token_generator.check_token(user, token):
        if request.method == 'POST':
            ...
            if new_password != confirm_password:
                messages.error(request, "Passwords do not match")
                return render(request, 'template/password_reset_confirm.html', {'uidb64': uidb64, 'token': token})
